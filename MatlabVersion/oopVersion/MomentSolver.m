classdef MomentSolver
    ...
    methods
        function obj = MomentSolver(energy, current, initialMoments)
            obj.energy = energy;
            obj.current = current;
        ...
        function [O_mat,N_mat] = CalcONMatrix(obj)
        ...
        function [FoM, FoMp, dQy, dPy, dEy, dLy] = GetFAndDF1(obj)
            ...
            f4_tmp = ( y(i,7) - 0.5*(komega^2)*y(i,1) + obj.perveance );
            ...
            dPy(2,1) = y(i,5);
            dPy(3,1) = y(i,6);
            dEy(1,1) = (k0^(-2))*e2*(f5_tmp)*(0.5*komega^2)*(-1) + (k0^(-2))*e1*(f4_tmp)*(-0.5*komega^2)*(-1);
            dEy(2,1) = -k0^(2)*y(i,2);
            dEy(3,1) = -k0^(2)*y(i,3);
            dQy(1,1) = (k0^(-2))*e2*(f5_tmp)*(-1) + (k0^(-2))*e1*(f4_tmp)*(-1);
            dQy(2,1) = -k0^(-2)*y(i,8);
            dQy(3,1) = -k0^(-2)*y(i,9);
            dLy(1,1) = (k0^(-2))*e2*(f5_tmp)*(-1)*(komega);
        ...
        function PlotBeamSize(obj)
            ...
            legend('\langle x^2 \rangle','\langle y^2 \rangle','K_{quad}','Location','southeast');
            grid on;
        end
